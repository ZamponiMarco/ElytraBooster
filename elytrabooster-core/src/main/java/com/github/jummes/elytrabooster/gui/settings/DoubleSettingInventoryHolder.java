...
public class DoubleSettingInventoryHolder extends SettingInventoryHolder {
	...
	public DoubleSettingInventoryHolder(BoosterManager<?> manager, ConfigurationSection section, String key,
	...
	protected void initializeInventory() {
		...
		registerClickConsumer(11, getModifyItem(-0.01, ElytraBooster.getInstance().getWrapper().skullFromValue(ARROW_LEFT_HEAD)), getConsumer(-0.01));
		registerClickConsumer(10, getModifyItem(-0.1, ElytraBooster.getInstance().getWrapper().skullFromValue(ARROW2_LEFT_HEAD)), getConsumer(-0.1));
		registerClickConsumer(9, getModifyItem(-1.0, ElytraBooster.getInstance().getWrapper().skullFromValue(ARROW3_LEFT_HEAD)), getConsumer(-1));
		registerClickConsumer(15, getModifyItem(+0.01, ElytraBooster.getInstance().getWrapper().skullFromValue(ARROW_RIGHT_HEAD)), getConsumer(+0.01));
		registerClickConsumer(16, getModifyItem(+0.1, ElytraBooster.getInstance().getWrapper().skullFromValue(ARROW2_RIGHT_HEAD)), getConsumer(+0.1));
		registerClickConsumer(17, getModifyItem(+1.0, ElytraBooster.getInstance().getWrapper().skullFromValue(ARROW3_RIGHT_HEAD)), getConsumer(+1));
		registerClickConsumer(13, getConfirmItem(), e -> {
			Booster booster = manager.getBooster(section.getName());
			manager.setParam(booster.getId(), key, String.valueOf(result));
	...
	private ItemStack getConfirmItem() {
		ItemStack item = ElytraBooster.getInstance().getWrapper().skullFromValue(SUBMIT);
		ItemMeta meta = item.getItemMeta();
