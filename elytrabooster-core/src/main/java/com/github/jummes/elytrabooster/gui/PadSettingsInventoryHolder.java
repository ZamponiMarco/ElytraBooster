...
import com.github.jummes.elytrabooster.gui.settings.enums.VisualEnumSettingInventoryHolder;
...
public class PadSettingsInventoryHolder extends ElytraBoosterInventoryHolder {
	...
	protected void initializeInventory() {
		...
		registerClickConsumer(3,
				getPadSetting(ElytraBooster.getInstance().getWrapper().skullFromValue(HORIZONTAL_VELOCITY_HEAD), "horizontalVelocity",
		...
		registerClickConsumer(5,
				getPadSetting(ElytraBooster.getInstance().getWrapper().skullFromValue(VERTICAL_VELOCITY_HEAD), "verticalVelocity",
		...
		registerClickConsumer(8,
				getPadSetting(ElytraBooster.getInstance().getWrapper().skullFromValue(TRAIL_HEAD), "trail", pad.getBoost().getTrail().getName()),
		...
		registerClickConsumer(17,
				getPadSetting(ElytraBooster.getInstance().getWrapper().skullFromValue(VISUAL_HEAD), "visual", pad.getVisual().getName()),
				getSettingConsumer(manager, section, "visual", pad.getVisual().getName(),
						VisualEnumSettingInventoryHolder.class));
		registerClickConsumer(13, getPadSetting(ElytraBooster.getInstance().getWrapper().skullFromValue(COOLDOWN_HEAD), "cooldown", pad.getCooldown()),
